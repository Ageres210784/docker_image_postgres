...
For example see thedocker-compose.yml.example and .env.example

## Sequencing

- change the versions of the postgres container and wal-g in the Dockerfile
- change container name and other parameters in docker-compose.yml (see docker-compose.yml.example)
- docker-compose build
- docker-compose up -d
- for periodic archiving, you need to configure the cron with the following task:
```bash
* * * * * /usr/local/bin/wal-g backup-push /var/lib/postgresql/12/main >> /var/log/postgresql/walg_backup.log 2>&1
````
- to clean up archives:
```bash
* * * * * /usr/local/bin/wal-g delete before FIND_FULL $(date -d '-1 days' '+\%FT\%TZ') --confirm >> /var/log/postgresql/walg_delete.log 2>&1
```